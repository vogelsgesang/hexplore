...
import React, { useRef, useState, useEffect, useLayoutEffect, CSSProperties, Ref, RefObject, useMemo, useCallback } from "react";
import ResizeObserver from 'resize-observer-polyfill';
import { DataGrid, HighlightRange, Range } from "./DataGrid"
import { AddressGutter } from "./AddressGutter"
import { HexViewerConfig, ColumnType, AddressGutterConfig, IntegerColumnConfig, IntegerDisplayMode, ColumnConfig } from "./HexViewerConfig";
...
const createIntegerRenderer = memoize(function createIntegerRenderer(width : 1|2|4, signed : boolean, littleEndian : boolean, display : IntegerDisplayMode) {
    ...
    switch(display) {
        ...
        case IntegerDisplayMode.Octal: base = 8; break;
        case IntegerDisplayMode.Decimal: base = 10; break;
        case IntegerDisplayMode.Hexadecimal: base = 16; break;
    }
...
function HexViewerColumn({dataView, columnConfig, lineWidth, firstRenderedLine, lastRenderedLine, highlightRanges, cursorPosition, setCursorPosition, selection, setSelection} : HexViewerColumnProps) {
    ...
    switch (columnConfig.columnType) {
        ...
        default: assertExhausted(columnConfig.columnType);
    }
    ...
    const gridProps = {
        ...
        setCursorPosition: useCallback((e) => setCursorPosition ? (setCursorPosition(e*elementWidth)) : undefined, [elementWidth]),
        selection : useMemo(() => selection ? { from: Math.floor(selection.from/elementWidth), to: Math.ceil(selection.to/elementWidth) } : undefined, [selection, elementWidth]),
        setSelection : useCallback((s : Range) => setSelection ? setSelection({ from : s.from*elementWidth, to: s.to*elementWidth }) : undefined, [setSelection, elementWidth]),
        highlightRanges: useMemo(() => highlightRanges?.map((h) => { return {...h, from: Math.floor(h.from/elementWidth), to: Math.ceil(h.to/elementWidth)}}), [highlightRanges, elementWidth]),
    };
    switch (columnConfig.columnType) {
        case ColumnType.AddressGutter: {
            const cc = columnConfig as AddressGutterConfig;
            return <AddressGutter
                {...scrollRelatedProps}
                offset={cc.offset}

...
export function FileOpener({setData} : FileOpenerProps) {
    ...
    let [loadState, setLoadState] = useState<LoadState>({state: "pristine"});
    ...
    function openURL(url : string) {
        ...
        .catch((e) => {
            ...
            window.setTimeout(() => setLoadState({"state": "error", "detail": `Unable to load "${url}": ${e.message}`}), 200);
        })
    }
    ...
    if (loadState.state == "loading") {
        ...
        if (loadState.state == "error") {
            renderedError = <Alert variant="danger" onClose={() => setLoadState({"state": "pristine"})} dismissible>{loadState.detail}</Alert>;
        }
        return (
            <div className="file-selection-page">
                {renderedError}
                ...
                <div className="source-alternatives">
                    <div className="source-alternative-url">
                        ...
                        <Button onClick={() => openURL(url)} size="sm" block>Open URL</Button>
                    </div>
                    <div>

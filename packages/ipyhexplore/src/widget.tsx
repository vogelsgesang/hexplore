...
import {
  DOMWidgetModel, DOMWidgetView, ISerializers, ManagerBase
...
export class HexViewerView extends DOMWidgetView {
  ...
  render() {
    ...
    this.model.on({
      ...
      "change:linewidth": () => { this.rebuildColumnConfig(); this.redraw(); },
      "change:columns": () => { this.rebuildColumnConfig(); this.redraw(); },
      "change:selection_from": () => { this.rebuildSelection(); this.redraw(); },
      "change:selection_to": () => { this.rebuildSelection(); this.redraw(); },
      "change:highlight_ranges": () => { this.rebuildHighlightRanges(); this.redraw(); },
    });
  ...
  rebuildHighlightRanges() {
    let rawRanges : Array<PyHightlightRanges> = this.model.get('highlight_ranges');
  ...
  redraw() {
    ...
    ReactDOM.render(
        <HexViewer
            ...
            setCursorPosition={pos => { this.model.set("cursor_position", pos); this.model.save_changes(); }}
